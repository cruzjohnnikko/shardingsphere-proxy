...
async function startStreaming() {
    try {
        ...
        if (result.success) {
            addMessage(result.message + ' (ID: ' + result.streamingId + ')', 'success');
        } else {
            addMessage(result.message, 'error');
            startStreamingBtn.disabled = false;
        }
    ...
    try {
        ...
        if (sourceResult.success) {
            ...
            document.getElementById('sourceDataTable').innerHTML = 
                `<tr><td colspan="3" style="color: #ef4444;">${sourceResult.message}</td></tr>`;
        }
        ...
        if (targetResult.success) {
            ...
            document.getElementById('targetDataTable').innerHTML = 
                `<tr><td colspan="3" style="color: #ef4444;">${targetResult.message}</td></tr>`;
        }
        ...
        document.getElementById('totalEventsCaptured').textContent = stats.totalEvents.toLocaleString();
        ...
        let unsyncedCount = Math.max(0, sourceRecords.length - targetRecords.length);
        ...
        document.getElementById('sourceRecordCount').textContent = 'Error';
        document.getElementById('targetRecordCount').textContent = 'Error';
        document.getElementById('unsyncedRecordCount').textContent = 'Error';
        document.getElementById('totalEventsCaptured').textContent = 'Error';
        document.getElementById('cdcJobStatus').textContent = 'API Error';
        document.getElementById('cdcJobStatus').className = 'status-indicator not-running';
    }
}
